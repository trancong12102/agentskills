#!/usr/bin/env python3
"""
Semantic codebase search via MorphLLM (through mcporter).

Usage:
    python3 codebase-search.py search <query> [repo_path] [--search-type TYPE] [--timeout N] [--dry-run]
...
Tips:
    Write queries as natural language questions — codebase search is an RL-trained
...
Examples:
    python3 codebase-search.py search "how does auth work"
    python3 codebase-search.py search "how does auth work" /path/to/repo
    python3 codebase-search.py search "where are deps resolved" --search-type node_modules
    python3 codebase-search.py search "trace the payment flow" --timeout 180
    python3 codebase-search.py search "how does auth work" --dry-run
"""
...
def main():
    ...
    if subcommand == "search":
        if len(sys.argv) < 3:
            fail("Usage: codebase-search.py search <query> [repo_path] [--search-type TYPE] [--timeout N] [--dry-run]")
