...
TanStack Table ships zero markup, styles, or DOM manipulation. It is a pure state machine:
you give it data and column definitions, it gives back a `table` object with getters
(`getHeaderGroups()`, `getRowModel()`) and state. You render whatever HTML/CSS you want.
...
const columns = [
  // Key-based accessor — TS infers value type
  columnHelper.accessor('firstName', {
    header: 'First Name',
    cell: (info) => info.getValue(),
  }),

  // Function-based accessor — must have explicit id
  columnHelper.accessor((row) => row.lastName, {
    id: 'lastName',
    header: () => <span>Last Name</span>,
  }),

  // Display column — no data, for action buttons
  ...
  columnHelper.group({
    header: 'Info',
    columns: [columnHelper.accessor('age', { header: 'Age' })],
  }),
]
```
...
Features are tree-shaken — only bundle what you use:
